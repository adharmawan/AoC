...
func main() {
	...
	fmt.Println("The answer is: ", findAnswer(inputArray))
	return
}

func findAnswer(sortedInputArray []string) string {
	for i := 0; i < len(sortedInputArray); i++ {
		for j := 0; j < len(sortedInputArray); j++ {
			if i != j && charDiff(sortedInputArray[i], sortedInputArray[j]) == 1 {
				return intersect(sortedInputArray[i], sortedInputArray[j])
			}
		}
	}
...
func charDiff(str1, str2 string) int {
	var count int
	for i := range str1 {
		if str1[i] != str2[i] {
			count = count + 1
...
func intersect(str1, str2 string) string {
	var str string
	for i := range str1 {
		if str1[i] == str2[i] {
